...
<div class="flex items-center text-gray-500">
    <form method="POST" action="/tweets/{{ $tweet->id }}/like" class="mr-4">
        @csrf

        <button type="submit">
            <span class="flex items-center text-sm {{ $tweet->isLikedBy(auth()->user()) ? 'text-blue-500' : 'text-gray-500' }}">
    ...
    <form method="POST" action="/tweets/{{ $tweet->id }}/like">
        ...
        <button type="submit">
            <span class="flex items-center text-sm {{ $tweet->isDislikedBy(auth()->user()) ? 'text-blue-500' : 'text-gray-500' }}">
                ...
                {{ $tweet->sum_dislikes ?: 0 }}
                {{--                    {{ $tweet->dislikes ? $tweet->dislikes->count() : 0 }}--}}
            </span>
        </button>
    </form>
</div>
